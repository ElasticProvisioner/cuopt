...
// Event types generated by B&B workers during BSP execution
enum class bb_event_type_t : int8_t {
  NODE_BRANCHED     = 0,  // Node was solved and branched into two children
  NODE_FATHOMED     = 1,  // Node was fathomed (cutoff or infeasible)
  NODE_INTEGER      = 2,  // Node has an integer feasible solution
  NODE_PAUSED       = 3,  // Node processing paused at horizon boundary
  NODE_INFEASIBLE   = 4,  // Node LP relaxation is infeasible
  NODE_NUMERICAL    = 5,  // Numerical issue encountered
  HEURISTIC_SOLUTION = 6  // Solution received from heuristics
};

// Payload for NODE_BRANCHED events
...
// Payload for NODE_INTEGER events (integer feasible solution found)
template <typename f_t>
struct integer_solution_payload_t {
  f_t objective_value;
  // Note: the full solution is stored separately, not in the event
};

// Payload for NODE_FATHOMED events
template <typename f_t>
struct fathomed_payload_t {
  f_t lower_bound;  // Bound at which node was fathomed
};

// Payload for NODE_PAUSED events
template <typename f_t>
struct paused_payload_t {
  f_t accumulated_vt;  // How much virtual time spent on this node so far
};

// Payload for HEURISTIC_SOLUTION events
template <typename f_t>
struct heuristic_solution_payload_t {
  f_t objective_value;
  size_t solution_index;  // Index into a solution storage vector
};

// A single event generated during BSP execution
template <typename i_t, typename f_t>
struct bb_event_t {
  bb_event_type_t type;
  double vt_timestamp;     // Virtual time when this event occurred
  int worker_id;           // Which worker generated this event
  i_t node_id;             // Node that generated this event
  int event_sequence;      // Sequence number within worker for tie-breaking

  // Payload union - only one is valid based on type
  union {
  ...
  // Comparison for deterministic sorting: (vt_timestamp, worker_id, event_sequence)
  ...
  static bb_event_t make_heuristic_solution(double vt,
                                            int worker,
                                            int seq,
                                            f_t objective,
                                            size_t sol_idx)
  {
    ...
    e.vt_timestamp   = vt;
    e.worker_id      = worker;
    e.node_id        = -1;  // Not associated with a B&B node
    e.event_sequence = seq;
...
// Batch of events from a single horizon period
template <typename i_t, typename f_t>
