...
{
  SpMM_benchmarks_context_t(detail::cusparse_sp_mat_descr_wrapper_t<i_t, f_t>& A,
    detail::cusparse_sp_mat_descr_wrapper_t<i_t, f_t>& A_T, int primal_size, int dual_size, size_t current_batch_size, raft::handle_t const* handle_ptr)
  ...
  detail::cusparse_dn_mat_descr_wrapper_t<f_t> x_descr;
  detail::cusparse_dn_mat_descr_wrapper_t<f_t> y_descr;
  
  int rows_primal = primal_size;
  int col_primal = current_batch_size;
  int ld_primal = (detail::use_row_row) ? current_batch_size : rows_primal;

  int rows_dual = dual_size;
  int col_dual = current_batch_size;
  int ld_dual = (detail::use_row_row) ? current_batch_size : rows_dual;
  ...
  detail::my_cusparsespmm_preprocess<f_t>(handle_ptr->get_cusparse_handle(),
  CUSPARSE_OPERATION_NON_TRANSPOSE,
  CUSPARSE_OPERATION_NON_TRANSPOSE,
  alpha.data(),
  A_T,
  ...
  detail::ping_pong_graph_t<i_t> ping_pong_graph;
};

template <typename i_t, typename f_t>
static double evaluate_node(detail::cusparse_sp_mat_descr_wrapper_t<i_t, f_t>& A, detail::cusparse_sp_mat_descr_wrapper_t<i_t, f_t>& A_T, i_t primal_size, i_t dual_size, int current_batch_size, int benchmark_runs, raft::handle_t const* handle_ptr)
{
...
template <typename i_t, typename f_t>
static int optimal_batch_size_handler(const detail::problem_t<i_t, f_t>& op_problem, int max_batch_size)
{
  if (max_batch_size == 1)
    return 1;
  ...
  // Init cuSparse views
  detail::cusparse_sp_mat_descr_wrapper_t<i_t, f_t> A;
  detail::cusparse_sp_mat_descr_wrapper_t<i_t, f_t> A_T;
