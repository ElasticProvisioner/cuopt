...
TEST_P(DeterministicBBTestFixture, deterministic_reproducibility)
{
  ...
  EXPECT_EQ(test_deterministic_bb(filename.c_str(), 3, num_threads, time_limit), CUOPT_SUCCESS);
}
INSTANTIATE_TEST_SUITE_P(c_api,
                         DeterministicBBTestFixture,
                         ::testing::Values(
                           // Low thread count
                           std::make_tuple("/mip/gen-ip054.mps", 4, 60.0),
                           // High thread count (high contention)
                           std::make_tuple("/mip/gen-ip054.mps", 128, 60.0),
                           // Different instance
                           std::make_tuple("/mip/bb_optimality.mps", 8, 60.0)));
